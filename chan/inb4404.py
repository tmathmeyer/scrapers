...
def main():
  ...
  parser.add_argument(
    'thread', nargs=1, help='url of the thread (or filename; one url per line)'
  )
  parser.add_argument(
    '-c',
    '--with-counter',
  ...
  parser.add_argument(
    '-d', '--date', action='store_true', help='show date as well'
  )
  parser.add_argument(
    '-l',
    '--less',
  ...
  parser.add_argument(
    ...
    action='store_true',
    help=
    'use thread names instead of the thread ids (...4chan.org/board/thread/thread-id/thread-name)'
  )
  parser.add_argument(
    '-r',
    '--reload',
  ...
  parser.add_argument(
    '-t', '--title', action='store_true', help='save original filenames'
  )
  parser.add_argument(
    '--no-new-dir',
    action='store_true',
    help='don\'t create the `new` directory'
  )
...
def download_thread(thread_link, args):
  ...
  if len(thread_link.split('/')) > 6:
    ...
    if args.use_names or os.path.exists(
      os.path.join(workpath, 'downloads', board, thread_tmp)
    ):
      thread = thread_tmp
  ...
  while True:
    try:
      ...
      for enum_index, enum_tuple in enumerate(regex_result):
        ...
        if not os.path.exists(img_path):
          ...
          if args.with_counter:
            output_text = '[' + str(regex_result_cnt).rjust(
              len(str(regex_result_len))
            ) + '/' + str(regex_result_len) + '] ' + output_text
      ...
      urllib.error.URLError, http.client.BadStatusLine,
...
def download_from_file(filename):
  ...
  while True:
    ...
    for link in [
      _f
      for _f in [line.strip() for line in open(filename) if line[:4] == 'http']
      if _f
    ]:
      ...
      process = Process(target=call_download_thread, args=(
        link,
        args,
      ))
      process.start()
    ...
    if args.reload:
      ...
      if not args.less:
        log.info(
          'Reloading ' + args.thread[0]
        )  # thread = filename here; reloading on next loop
    else:
      break
